# 🍳 API рецептов на Node.js + Express

REST API для управления рецептами с категориями и ингредиентами. Реализовано на Node.js с использованием Express, включает валидацию, обработку ошибок и документацию Swagger.

## 📋 Функциональность

### Основные возможности

- **CRUD операции** для рецептов и категорий
- **Валидация данных** с использованием Zod
- **Пагинация и фильтрация** для списков
- **Поиск** по названию, описанию и ингредиентам
- **Документация OpenAPI** с Swagger UI
- **Централизованная обработка ошибок**
- **Временное хранение данных** в памяти

### Маршруты API

- `GET    /api/v1/recipes` - список рецептов с пагинацией
- `GET    /api/v1/recipes/:id` - детали рецепта
- `POST   /api/v1/recipes` - создание рецепта
- `PUT    /api/v1/recipes/:id` - обновление рецепта
- `PATCH  /api/v1/recipes/:id` - частичное обновление
- `DELETE /api/v1/recipes/:id` - удаление рецепта
- `GET    /api/v1/categories` - список категорий
- `GET    /api/v1/recipes/search` - поиск рецептов
- `GET    /api/v1/recipes/stats` - статистика

## 🚀 Быстрый старт

### 1. Установка зависимостей

```bash
npm install
```

### 2. Настройка окружения

Скопируйте файл .env.example в .env:

```bash
cp .env.example .env
```

### 3. Запуск в режиме разработки

```bash
npm run dev
```

Сервер будет доступен по адресу: http://localhost:3000

### 4. Документация API

После запуска сервера откройте в браузере:

```text
http://localhost:3000/api/v1/docs
```

## 🧪 Тестирование

### Запуск всех тестов

```bash
npm test
```

### Запуск тестов в режиме watch

```bash
npm run test:watch
```

### Покрытие кода тестами

```bash
npm test -- --coverage
```

## 🔧 Доступные скрипты

```bash
npm start - запуск в production режиме

npm run dev - запуск с nodemon (автоперезагрузка)

npm test - запуск тестов

npm run lint - проверка кода ESLint

npm run format - форматирование кода Prettier
```

## 🌐 Примеры запросов

### Получение списка рецептов

```bash
curl -X GET "http://localhost:3000/api/v1/recipes?page=1&limit=10"
Создание рецепта:
bash
curl -X POST "http://localhost:3000/api/v1/recipes" \
-H "Content-Type: application/json" \
-d '{
"title": "Новый рецепт",
"description": "Описание рецепта",
"category": "cat1",
"difficulty": "легко",
"time": 30,
"servings": 4,
"ingredients": [
{"name": "Ингредиент 1", "amount": "100 г"}
],
"steps": ["Шаг 1", "Шаг 2"]
}'
```

### Поиск рецептов

```bash
curl -X GET "http://localhost:3000/api/v1/recipes/search?q=спагетти"
```

### 📊 Статус коды

- 200 - Успешный запрос

- 201 - Ресурс создан

- 400 - Некорректный запрос

- 404 - Ресурс не найден

- 409 - Конфликт данных

- 422 - Ошибка валидации

- 500 - Внутренняя ошибка сервера

## 🛠️ Технологии

- Node.js - среда выполнения

- Express - веб-фреймворк

- Zod - валидация данных
