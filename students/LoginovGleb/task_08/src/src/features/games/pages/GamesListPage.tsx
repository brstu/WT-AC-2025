...
import { useGetGamesListQuery } from '../api/gamesApi';
  ...
  if (error) {
    return (
      <div className="error-container">
        ...
        <p>{'message' in error ? String(error.message) : 'Не удалось загрузить список игр'}</p>
      </div>
    );
  }
              ...
              <Card key={game.id}>
                <Link to={`/games/${game.id}`} className="game-card-link">
                  ...
                  <p className="game-rating">
                    Рейтинг: <span className="rating">⭐ {game.rating}/10</span>
                  </p>
                  <p className="game-status">
                    Статус: <span className={`status-${game.status}`}>{game.status}</span>
                  </p>
                </Link>
              </Card>
            ))}
