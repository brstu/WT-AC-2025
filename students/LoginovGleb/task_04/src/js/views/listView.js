...
function renderListContent(result, filters) {
    ...
    const html = `
        ...
        <div class="search-bar" role="search">
            ...
            <div class="filter-group">
                ...
                <select id="category-filter" class="form-select" aria-label="Выберите категорию">
                    ...
                    ${categories.map(cat => `
                        <option value="${escapeHtml(cat)}" ${category === cat ? 'selected' : ''}>
                            ${escapeHtml(cat)}
                        </option>
                    `).join('')}
                </select>
        ...
        ${events.length === 0 ? renderEmptyState(search, category) : renderEventsList(events)}
...
function renderEventCard(event) {
    ...
    return `
        <article class="card card--clickable" role="listitem" data-event-id="${event.id}">
            <img 
            ...
            <div class="card-body">
                ...
                <h2 class="card-title">
                    <a href="#/items/${event.id}" data-prefetch="${event.id}">${escapeHtml(event.title)}</a>
                </h2>
                <p class="card-text">${escapeHtml(truncateText(event.description, 120))}</p>
                <div class="card-meta">
